...
var y = new[] { 1, 2 };
throw new Exception();
var z = new MyClass();   // new + 型別名稱：有空格
MyClass obj = new();     // target-typed new：無空格
...
逗號前後應有正確的空白。逗號後應接一個空格，下列情況例外：行尾、`typeof` 中開放泛型型別、字串插補對齊（`$"{x,3}"`）、條件前置處理器指令後。逗號前不可有空格，且不可出現在行首。

---

## SA1002 — SemicolonsMustBeSpacedCorrectly

分號前後應有正確的空白。分號後應接一個空格，下列情況例外：行尾、後接右括號。分號前不可有空格，除非是行首。
...
單行註解（`//`）內容前應以單一空格開頭。例外：以 `////` 開頭的 comment-out 程式碼可省略空格。
...
運算子多載方法中的 `operator` 關鍵字後應接空格。

---

## SA1008 — OpeningParenthesisMustBeSpacedCorrectly

左括號前後應有正確的空白。左括號前通常不可有空格，除非是行首、後接 `if`/`while`/`for` 等關鍵字，或在運算式中跟在運算子後面。左括號後不可有空格，除非是行尾。

---

## SA1009 — ClosingParenthesisMustBeSpacedCorrectly

右括號前後應有正確的空白。右括號前不可有空格。右括號後通常應接一個空格，例外：cast 後、後接特定運算子（`+`、`-`、`:`）。右括號後若有空格，下一個非空白字元不可是括號、方括號、分號或逗號。

---

## SA1010 — OpeningSquareBracketsMustBeSpacedCorrectly

左方括號前後不應有空白。左方括號前後均不可有空格，除非是行首或行尾。

---

## SA1011 — ClosingSquareBracketsMustBeSpacedCorrectly

右方括號前後應有正確的空白。右方括號前不可有空格，除非是行首。右方括號後應有空格，下列情況除外：行尾、後接右括號或左括號、後接逗號或分號、字串插補對齊/格式化、特定運算子、nullable 標註（`[]?`）、null-forgiving 運算子（`[0]!`）、函式指標呼叫慣例（`[Cdecl]<`）。

---

## SA1012 — OpeningBracesMustBeSpacedCorrectly

左大括號前後應有正確的空白。左大括號前應有一個空格，除非是行首或前接左括號（此時不加空格）。左大括號後應有一個空格，除非是行尾。

---

## SA1013 — ClosingBracesMustBeSpacedCorrectly

右大括號前後應有正確的空白。右大括號後應有一個空格，除非是行尾、後接右括號/逗號/分號，或後接 null-forgiving 運算子（`}!`）。右大括號前應有一個空格，除非是行首。

---

## SA1014 — OpeningGenericBracketsMustBeSpacedCorrectly

泛型左角括號（`<`）前後不應有空白。泛型左角括號前後均不可有空格，除非是行首或行尾。同樣適用於 C# 9 函式指標參數清單的 `<`（如 `delegate*<int, void>`）。

---

## SA1015 — ClosingGenericBracketsMustBeSpacedCorrectly

泛型右角括號（`>`）前後應有正確的空白。右角括號前不可有空格，除非是行首。右角括號後應接左括號、右括號、另一個右角括號、nullable 符號、行尾，或單一空格（但不可是空格後接左括號）。

---

## SA1016 — OpeningAttributeBracketsMustBeSpacedCorrectly

屬性左方括號（`[`）後不應有空白。屬性左方括號後不可有空格，除非是行尾。

---

## SA1017 — ClosingAttributeBracketsMustBeSpacedCorrectly

屬性右方括號（`]`）前不應有空白。屬性右方括號前不可有空格，除非是行首。

---

## SA1018 — NullableTypeSymbolsMustNotBePrecededBySpace

nullable 型別符號（`?`）前不應有空格。nullable 型別符號前不可有空格，除非是行首。適用於 nullable value type 與 C# 8 nullable reference type。

---

## SA1019 — MemberAccessSymbolsMustBeSpacedCorrectly

成員存取符號（`.`、`?.`）兩側不應有空白。成員存取符號兩側均不可有空格，除非是行首。也適用於緊接 null-forgiving 運算子後的存取，如 `value!.Property`。
...
負號（一元 `-`）前後應有正確的空白。負號前應有一個空格，除非前接左方括號、括號、插補字串大括號、行首，或在字串插補對齊中。負號後不可有空格，且不可是行尾。

---

## SA1022 — PositiveSignsMustBeSpacedCorrectly

正號（一元 `+`）前後應有正確的空白。規則與 SA1021 相同，方向為正號。
...
程式碼中不應有連續多個空白字元。行首/行尾，或逗號、分號後方除外。

---

## SA1026 — CodeMustNotContainSpaceAfterNewKeywordInImplicitlyTypedArrayAllocation

隱含型別陣列配置中，`new` 或 `stackalloc` 關鍵字與左方括號之間不應有空格。

---

## SA1027 — UseTabsCorrectly

程式碼中 tab 或空格的使用應與專案設定一致。預設不允許 tab 字元（可透過 `stylecop.json` 調整）。無論設定為何，非行首位置的 tab 一律違規。例外：字串字面值、字元字面值、comment-out 程式碼（`////`）、停用的程式碼區塊。

---

## SA1028 — CodeMustNotContainTrailingWhitespace

行尾不應有多餘的空白字元（空格、tab 等）。適用於一般程式碼、行/區塊/文件註解、前置處理器指令；不適用於逐字字串字面值（`@""`）及停用的程式碼區塊。
